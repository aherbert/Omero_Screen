...
from omero_screen.image_analysis_nucleus import NucImage
...
def test_image_create(omero_conn):
    ...
    assert image.img_dict["DAPI"].shape == (1020, 1020), "problem with image_dict"
...
def test_image_load(omero_conn):
    ...
    assert image.n_mask.max() == 196, "Nuclear Segmentation is incorrect"
    assert image.img_dict["DAPI"].shape == (1020, 1020), "Mask is not binary"

...
def test_image_data(omero_conn):
    ...
    assert (
        round(image_df.loc[0, "intensity_max_DAPI_nucleus"].mean(), 3) == 15553.086
    ), "Nuclear Segmentation is incorrect"
    assert len(image_df) == 183, "The length of the image df is incorrect"
    assert (
        round(image_data.image_df["intensity_mean_EdU_nucleus"].mean(), 3) == 3612.586
    ), "The mean of the EdU intensity is incorrect"
    assert (
        round(image_data.quality_df["intensity_median"].mean(), 3) == 1841.78
    ), "The median of the EdU intensity in the quality df is incorrect"

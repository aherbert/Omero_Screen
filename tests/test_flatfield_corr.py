...
def test_example_image_attach(omero_conn):
    ...
    pdf_annotations = [ann for ann in annotations if
                       isinstance(ann, omero.gateway.FileAnnotationWrapper) and ann.getFile().getName().endswith('.pdf')]
    ...
    assert 'DAPI_flatfield_check.pdf' in pdf_list, "Example Images are not named correctly"

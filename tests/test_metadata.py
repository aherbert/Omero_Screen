...
from omero_screen.metadata import MetaData
...
def test_excel_no_map(omero_conn):
    plate_obj = omero_conn.getObject("Plate", 2)  # or some other plate ID
    attach_excel_file(
        plate_obj, "./data/metadata.xlsx", omero_conn
    )  # or some real Excel data
    instance = MetaData(omero_conn, plate_id=2)
    assert instance.channels == {
        ...
        "p21": 2,
        "EdU": 3,
    }, "Channels from excelfile not as expected"
...
def test_excel_and_map(omero_conn):
    ...
    add_map_annotations(
        plate_obj,
        [["Drug Name", "Monastrol"], ["Concentration", "5 mg/ml"]],
        conn=omero_conn,
    )
...
def test_metadata_new(omero_conn):
    """Test if the absence of well metadata is spotted"""
    plate_obj = omero_conn.getObject(
        "Plate", 2
    )  # or some other plate ID  # or some other plate ID
    attach_excel_file(plate_obj, "./data/metadata.xlsx", omero_conn)
    instance = MetaData(omero_conn, plate_id=2)

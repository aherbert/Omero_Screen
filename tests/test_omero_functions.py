...
from omero_screen.omero_functions import load_data, delete_excel_attachments, add_map_annotation, delete_map_annotations
from omero.gateway import FileAnnotationWrapper, MapAnnotationWrapper
def test_load_data_valid_plate_and_file(omero_conn):
    """
    Tests the load_data function with a valid plate id and existing file.
    """
    plate_id = 2  # use actual plate id from your test server
    file_path = "./data/metadata.xlsx"  # ensure this file exists

    load_data(plate_id, file_path, conn=omero_conn)
...
def test_add_map_annotation(omero_conn):
    ...
    map_annotations = [ann for ann in plate.listAnnotations()
    ...
    added_annotation = map_annotations[-1]  # assuming the last annotation is the one added
    assert added_annotation.getValue() == key_value, "The added map annotation does not have the correct key-value pair."
...
def test_delete_map_annotations(omero_conn):
    ...
    map_annotations_after = [ann for ann in plate.listAnnotations()
                             if isinstance(ann, MapAnnotationWrapper)]
